...
import org.apache.spark.sql.catalyst.{expressions, DefinedByConstructorParams, InternalRow, ScalaReflection, WalkedTypePath}
...
private[sqlmacros] object ScalaSubtypeLock

/**
 * [[ScalaReflection]] utilities used within a macro evaluation.
 * - use the [[Utils.getSparkClassLoader]] for the runtime mirror, so all Spark classes are
 *   mirrored. Using the Thread classloader doesn't work; within macro evaluation thread
 *   classloader classpath is just scala and java jars.
 *
 * @param c
 */
private[sqlmacros] object MacrosScalaReflection extends ScalaReflection {
  val universe: scala.reflect.runtime.universe.type = scala.reflect.runtime.universe

  override val mirror: universe.Mirror = {
    universe.runtimeMirror(Utils.getSparkClassLoader)
  }
   ...
   */
  private[sqlmacros] def isSubtype(tpe1: `Type`, tpe2: `Type`): Boolean = {
    ScalaSubtypeLock.synchronized {
      tpe1 <:< tpe2
    }
  }

  private[sqlmacros] def dataTypeFor(tpe: `Type`): DataType = cleanUpReflectionObjects {
    tpe.dealias match {
